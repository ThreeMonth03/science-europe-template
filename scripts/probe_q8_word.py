...
import subprocess
from probe_budget_word import ROOT, IMAGE, RUNNER
    ...
    if isinstance(before, dict) and isinstance(after, dict):
        if before.get('t') == after.get('t') == 'Div':
            ...
            if left[0] == ['', [], []] and right[0] == ['', [], [STYLE]]:
                assert left[1] == right[1] and len(left[1]) == 1
...
def main():
    ...
    matrix = cases(); html = ''.join('<div id="'+name+'">'+body+'</div>' for name, body, _ in matrix)
    results = []
    ...
    report = {'passed': True, 'release_acceptance': False, 'rows': rows, 'worker_image': IMAGE,
              ...
              'limits': ['AST with fixed Pandoc, not Microsoft Word layout acceptance',
